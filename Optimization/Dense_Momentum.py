...
class DenseLayer:
    ...
    def update_weights_momentum(self, beta, lr):
        # 2. Cập nhật quán tính cho WEIGHTS
        self.weight_velocity = beta * self.weight_velocity + lr * self.dW
        self.weights = self.weights - self.weight_velocity
        
        # 3. Cập nhật quán tính cho BIASES
        self.biases_velocity = beta * self.biases_velocity + lr * self.dB
...
for epoch in range(EPOCHS):
    ...
    layer1.update_weights_momentum(0.9, lr)
    layer2.update_weights_momentum(0.9, lr)

    if epoch % 1000 == 0:
        print("Epoch:", epoch, "Losses: ", loss, "- Accuracy: ", accuracy)
